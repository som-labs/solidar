 ...
 * v 22/01/2024 - Change new cost propagation to all Zona Común participation
 */
...
var distribuirCosteZonaComun = [
  {
    ...
    action: function (e, cell) {
      ...
      for (let rowData of selectedData) {
        if (rowData.grupo !== 'Zonas Comunes') {
          //No se puede asignar coste de una zona comun a otra
...
async function inicializaEventos() {
  ...
  document.getElementById('botonDescargaReparto').addEventListener('click', function () {
    _tablaReparto.download('csv', 'reparto-' + TCB.nombreProyecto + '.csv', {
      delimiter: ';',
      bom: true,
    })
  })
  ...
  document
    .getElementById('botonRepartoParticipacion')
    .addEventListener('click', repartoParticipacion)

  document.getElementById('botonRepartoConsumo').addEventListener('click', repartoConsumo)

  document
    .getElementById('botonRepartoParitario')
    .addEventListener('click', repartoParitario)

  document
    .getElementById('nuevoPrecioInstalacion')
    .addEventListener('change', (e) => cambiaPrecioInstalacion(e))
...
async function muestraTablas() {
  ...
  TCB._tablaReparto =
    TCB._tablaReparto ??
    new Tabulator(divReparto, {
      columnDefaults: { headerTooltip: (e, col) => UTIL.hdrToolTip(e, col) },
      selectable: true,
      selectableRangeMode: 'click',
      //groupBy:"idPuntoConsumo",
      groupHeader: function (value, count) {
        //genera cabecera de los grupos. Muestra direccion del punto de consumo
      ...
      columns: [
        ...
        /*         {title:"Nombre", field:"nombreFinca", frozen: true, headerFilter:"input"}, */
        {
          ...
          field: 'nombreFinca',
          headerFilter: 'input',
          contextMenu: seleccionaTodoVisible,
        },
        {
          title: 'Grupo',
          field: 'grupo',
          hozAlign: 'center',
          editor: 'list',
          editorParams: Finca.getGrupos,
          titleDownload: 'grupo',
          headerFilter: 'input',
        },
        {
          title: 'Coef Propiedad',
          field: 'participacion',
          hozAlign: 'right',
        ...
        },
        {
          ...
          headerWordWrap: true,
          formatter: '_formatoValor',
          mutator: UTIL.round2Decimales,
          accessorDownload: UTIL.round2Decimales,
        },
        {
          title: 'Consumo',
          field: 'cTotalAnual',
        ...
        },
        {
          ...
          hozAlign: 'right',
          formatter: '_formatoValor',
          cellEdited: (cell) => cambioCoefEnergia(cell),
          headerMenu: opcionesReparto,
          mutator: UTIL.round2Decimales,
          accessorDownload: UTIL.round2Decimales,
        },
        {
          title: 'Producción',
          field: 'produccionTotal',
          hozAlign: 'right',
          formatter: '_formatoValor',
          accessorDownload: UTIL.round2Decimales,
          topCalc: 'sum',
        ...
        },
        {
          ...
          formatter: '_formatoValor',
          accessorDownload: UTIL.round2Decimales,
          contextMenu: distribuirCosteZonaComun,
        },
        {
          title: 'Coef Inversión',
          field: 'coefInversion',
          editor: 'number',
          topCalc: 'sum',
          topCalcFormatter: '_formatoValor',
          hozAlign: 'right',
          formatter: '_formatoValor',
          cellEdited: (cell) => cambioCoefInversion(cell),
          headerMenu: opcionesReparto,
          mutator: UTIL.round2Decimales,
          accessorDownload: UTIL.round2Decimales,
        },
        ...
        {
          titleFormatter: grafIcon,
          formatter: grafIcon,
          width: 40,
          hozAlign: 'center',
          headerSort: false,
          cellClick: (evt, cell) => informeEconomico(cell),
        },
        {
          titleFormatter: infoIcon,
          formatter: infoIcon,
          width: 40,
          hozAlign: 'center',
          headerSort: false,
          clickPopup: (evt, cell) => showEconomico(cell),
        },
      ],
    })
...
async function prepara() {
  ...
  UTIL.preparaInput(
    'nuevoPrecioInstalacion',
    cambiaPrecioInstalacion,
    TCB.produccion.precioInstalacion,
  )
  UTIL.muestra(
    'nuevoPrecioInstalacion',
    UTIL.formatoValor('nuevoPrecioInstalacion', TCB.produccion.precioInstalacion),
  )
  ...
  for (let finca of TCB.Participes) {
    ...
    if (
      finca.nombreTipoConsumo !== undefined &&
      finca.nombreTipoConsumo !== 'Participe sin consumo'
    ) {
    ...
    if (finca.grupo === 'Zonas Comunes') {
      ...
      if (!_colZC) {
        ...
        await _tablaReparto.addColumn(
          {
            title: finca.nombreFinca,
            field: finca.nombreFinca,
            formatter: (cell) => formatoNuevaColumna(cell),
            hozAlign: 'right',
            topCalc: 'sum',
            topCalcFormatter: (cell) => UTIL.n_formatoValor(cell, 'dinero'),
            minWidth: 20,
            accessorDownload: UTIL.round2Decimales,
          },
          true,
  ...
  document.getElementById('checkParticipacion').innerText = TCB.i18next.t(
    'botonCheckParticipacion_LBL',
    { participacion: coefPaticipacionTotal.toFixed(2) },
  )
  ...
  for (let finca of TCB.Participes) {
    if (finca.grupo === 'Zonas Comunes') {
      ...
      finca.coefEnergia =
        ((remanente * finca.participacion) / coefParticipacionConsumo) * 100
      finca.coefInversion = finca.coefEnergia
    } else {
...
function repartoParticipacion() {
  ...
  for (let finca of TCB.Participes) {
    let remanente = 1 - energiaPreAsignada / 100
    if (finca.grupo !== 'Zonas Comunes')
      finca.coefEnergia = remanente * (finca.participacion / coefPaticipacionTotal) * 100
  }
  validaResultados()
}
...
function cambioCondicionHucha(cell) {
  ...
  TCB.graficos.gestionEconomico_BalanceEconomico(
    'graf_ecoFinca',
    actFinca.economico,
    actFinca,
  )
...
function actualizaEstadoEnergia() {
  ...
  btnEnergia.innerHTML = TCB.i18next.t('botonCheckEnergia_LBL', {
    coefEnergia: UTIL.formatoValor('porciento', sumaCoefEnergia),
  })
  if (Math.round(Math.abs(100 - sumaCoefEnergia) > _epsilon)) {
    ...
    _tablaReparto.getColumn('coefEnergia').getElement().style.backgroundColor =
      'rgb(200, 249, 233)'
    return true
  }
}
...
function actualizaEstadoInversion() {
  ...
  for (let finca of TCB.Participes) {
    if (finca.grupo === 'Zonas Comunes') {
      ...
      inversionAsignada += TCB.Participes.reduce((a, b) => {
        return a + b[finca.nombreFinca]
      }, 0)
    } else {
  ...
  if (Math.round(Math.abs(inversionTotal - inversionAsignada)) > _epsilon) {
    ...
    _tablaReparto.getColumn('ahorroFincaAnual').getElement().style.backgroundColor =
      'rgb(200, 249, 233)'
    return true
  }
}
...
function informeEconomico(cell) {
  if (valida()) {
    //Verificamos que ya se ha realizado algún reparto
...
function cambiaPrecioInstalacion(evento) {
  ...
  const correccionPrecioInstalacion =
    parseFloat(evento.target.value) / TCB.produccion.precioInstalacion
  ...
  // 20240122 - Change loop to include cost modification into participation of each Zona Comun on each Finca
  TCB.Participes.forEach((finca) => {
    if (finca.grupo !== 'Zonas Comunes') {
      finca.coste = (TCB.produccion.precioInstalacion * finca.coefEnergia) / 100
    } else {
      TCB.Participes.forEach((nFinca) => {
        nFinca[finca.nombreFinca] *= correccionPrecioInstalacion
      })
    }
  })
  // End change
