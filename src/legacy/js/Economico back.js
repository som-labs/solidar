      ...
      for (let dia=0; dia < 365; dia++) {
        ...
        for (let hora = 0; hora < 24; hora++) {
          ...
          if (finca.balance.diaHora[dia][hora] < 0) {  //Aportamos energia a la red de distribución
            ...
            this.diaHoraPrecioConPaneles[dia][hora] += finca.balance.diaHora[dia][hora] * this.diaHoraTarifaConPaneles[dia][hora] * coefImpuesto;
            this.idxTable[dia].compensado += this.diaHoraPrecioConPaneles[dia][hora];
          } else { //Demandamos energia de la red de distribucion
            this.diaHoraTarifaConPaneles[dia][hora] = this.diaHoraTarifaOriginal[dia][hora];
            this.diaHoraPrecioConPaneles[dia][hora] = finca.balance.diaHora[dia][hora] * this.diaHoraTarifaConPaneles[dia][hora] * coefImpuesto;
            this.idxTable[dia].ahorradoAutoconsumo += TCB.produccion.diaHora[dia][hora] * finca.coefEnergia / 100 * this.diaHoraTarifaOriginal[dia][hora] * coefImpuesto;
          }
...
/* 
  ...
  correccionExcedentes(coefHucha, cuotaHucha) {
    ...
    for (let i = 0; i < 12; i++) {
      this.consumoConPlacasMensual[i] += _cuotaHucha;
      if (this.consumoConPlacasMensual[i] < 0) { //el excedente supera lo que hay que pagar
        this.perdidaMes[i] = -this.consumoConPlacasMensual[i]; //en principio no se compensa y es perdida
 ...
 */
  calculoFinanciero(coefEnergia, coefInversion) {
    ...
    if ((TCB.consumo.cTotalAnual / TCB.produccion.pTotalAnual) * 100 < 80 || tipoSubvencionEU === 'Sin') {
      valorSubvencionEU = 0;
    } else {
      if ( TCB.produccion.potenciaTotal <= 10) {
        valorSubvencionEU = TCB.subvencionEU[tipoSubvencionEU]['<=10kWp'] * TCB.produccion.potenciaTotal * coefInversion / 100;
      } else {
        valorSubvencionEU = TCB.subvencionEU[tipoSubvencionEU]['>10kWp'] * TCB.produccion.potenciaTotal * coefInversion / 100;
      }
    }
    ...
    if (coefInversion === 100) { //Vamos aguardar estas variables en TCB para el calculo económico global
