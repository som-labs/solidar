...
function inicializaEventos () {
  ...
  document.getElementById("localizacion").addEventListener("change", async function handleChange1() {
...
/*     {source: new ol.source.XYZ({
      url: 'https://api.mapbox.com/styles/v1/svenpt/cjsbq6vq716ye1fpgw10kvitp.html/tiles/256/{z}/{x}/{y}?access_token=MAPBOX_TOKEN_REMOVED'
    })} */
  );
  OSM.set('name', 'OSM');
  ...
  const vector = new ol.layer.Vector({ source: TCB.origenDatosSolidar,
  ...
  if (TCB.modoActivo === INDIVIDUAL) { //Solo se dibujan bases por lo que podemos dejar el comando activo
    geometriaActiva = {'nombre':'BaseSolar', 'tipo': 'Polygon'};
    divBaseSolar.style.display = 'block';
  } else {
...
async function importa( datosImportar) {
  ...
  datosImportar.BaseSolar.forEach( (base) => {
    ...
    const label = TCB.origenDatosSolidar.getFeatureById("BaseSolar.label." + base.idBaseSolar);
    ...
    const markerAcimut = TCB.origenDatosSolidar.getFeatureById("BaseSolar.symbol."+base.idBaseSolar);
    if (markerAcimut) {
    ...
    tbase.instalacion = new Instalacion({paneles:base.instalacion.paneles, potenciaUnitaria: base.instalacion.potenciaUnitaria});
  ...
  // Cálculo del CO2 equivalente a la producción anual de toda la instalación
    TCB.CO2AnualRenovable = TCB.conversionCO2[TCB.territorio].renovable * TCB.produccion.pTotalAnual;
    TCB.CO2AnualNoRenovable = TCB.conversionCO2[TCB.territorio].norenovable * TCB.produccion.pTotalAnual;
  }
  ...
  datosImportar.PuntoConsumo.forEach( (punto) => {
    ...
    if (TCB.modoActivo !== INDIVIDUAL) {
      ...
      const label = TCB.origenDatosSolidar.getFeatureById("PuntoConsumo.label." + punto.idPuntoConsumo);
      UTIL.setLabel (label, punto.nombrePuntoConsumo, TCB.baseLabelColor,TCB.baseLabelBGColor);
    }
...
function muestraTablas() {
    ...
    var deleteIcon = function(){ return "<i class='fa fa-trash-o'></i>"};
    var acimutIcon = function(){ return "<i class='fa fa-compass'></i>"};
    ...
    if (TCB.modoActivo !== INDIVIDUAL) {
      _tablaPuntoConsumo = _tablaPuntoConsumo ?? new Tabulator(divPuntoConsumo, {
        ...
        columns:[
            ...
            {title:"Direccion Postal", field:"direccion", hozAlign:"left", formatter:noEditable},
            {title:"Territorio", field:"territorio", hozAlign:"center", formatter:noEditable},
            {field: "botonBorraBase", titleFormatter: deleteIcon, 
            formatter: deleteIcon, width:40, hozAlign:"center", headerSort:false, cellClick: (evt, cell) => borraObjeto( cell)}
        ]
      });
      _tablaPuntoConsumo.on("tableBuilt", function(){Idioma.i18nTitulosTabla(_tablaPuntoConsumo)});
    }
  
    // Creamos tabla para gestionar las bases
    _tablaBaseSolar = _tablaBaseSolar ?? new Tabulator(divBaseSolar, {
      columnDefaults: {headerTooltip: (e, col) => UTIL.hdrToolTip(e, col)},
      height: "100%",
      layout: "fitColumns", 
      index: "idBaseSolar", 
      data: TCB.BaseSolar,
      columns:[
       {title: "Nombre", field:"nombreBaseSolar", hozAlign:"left", editor: "input", cellEdited: (cell) => UTIL.cambioValor(cell, true)},
       {title: "Lon-Lat", field:"lonlatBaseSolar", hozAlign:"center", formatter: noEditable},
       {title: "Area en mapa", field:"areaMapa", hozAlign:"right", formatter: noEditable},
       {title: "Inclinacion Tejado", field:"inclinacionTejado", hozAlign:"right", formatter: "_formatoValor", editor: "input", cellEdited: (cell) => inclinacionTejado(cell)},
       {title: "Area corregida", field:"areaReal", hozAlign:"right", formatter: noEditable},
       {title: "Max. kWp", field:"potenciaMaxima", hozAlign:"right", formatter: noEditable},
       {title: "Inclinacion del panel", field:"inclinacionPaneles",  hozAlign:"right", formatter: "_formatoValor",editor: "input", cellEdited: (cell) => inclinacionPaneles(cell)},
       {title: "Inclinación óptima", field:"inclinacionOptima", hozAlign:"center", headerSort:false, formatter: "tickCross", cellClick: (evt, cell)=>inclinacionOptima(cell)},
       {title: "Orientación (Acimut)", field:"inAcimut", formatter: "_formatoValor",  hozAlign:"right", editor: "input", cellEdited: (cell) => acimutBaseSolar(cell, "INPUT")},     
       {field: "botonAcimut", titleFormatter: acimutIcon, 
       formatter: acimutIcon, width:40, hozAlign:"center", headerSort:false, cellClick: (evt, cell) => acimutBaseSolar(cell, "GEO") },
       {title: "Angulos óptimos", field:"angulosOptimos", hozAlign:"center", formatter:"tickCross", cellClick: (evt,cell) => angulosOptimos(cell)},
       {field: "botonBorraBase", titleFormatter: deleteIcon, 
       formatter: deleteIcon, width:40, hozAlign:"center", headerSort:false, cellClick: (evt, cell) => borraObjeto( cell)}
      ]
    });
    _tablaBaseSolar.on("tableBuilt", function(){Idioma.i18nTitulosTabla(_tablaBaseSolar)});
}
...
async function construirBaseSolar ( geoBaseSolar) {
  ...
  /*     const largo1 = UTIL.distancia(puntos[0], puntos[1]);
    ...
    nuevaBaseSolar.nombreBaseSolar =TCB.nombreProyecto + ' Base '+ nuevaBaseSolar.idBaseSolar;
    ...
    nuevaBaseSolar.potenciaMaxima = (nuevaBaseSolar.areaMapa / TCB.parametros.conversionAreakWp);
    ...
    nuevaBaseSolar.geometria = {
      ...
      "label": nuevoLabel ("BaseSolar.label." + TCB.featIdUnico, puntoAplicacion, 
...
// En el modo INTERACTIVO no existe PuntoConsumo, en modo COLECTIVO puede existir solo 1, en COMUNIDAD puede haber muchos
  ...
  if (identificacionParcela.codigo !== 0) { //Si no hay datos de catastro
    if (confirm( TCB.i18next.t( "mapa_MSG_errorCatastroXY", {descripcion: identificacionParcela.descripcion}))) { //Pedimos confirmacion
  ...
  nuevoPuntoConsumo.nombrePuntoConsumo = TCB.nombreProyecto + " PC:" + nuevoPuntoConsumo.idPuntoConsumo;
  nuevoPuntoConsumo.lonlatPuntoConsumo = puntoAplicacion_4326[0].toFixed(4) + "," + puntoAplicacion_4326[1].toFixed(4);
  ...
  nuevoPuntoConsumo.geometria = {
    ...
    "label": nuevoLabel ("PuntoConsumo.label." + TCB.featIdUnico, puntoAplicacion, nuevoPuntoConsumo.nombrePuntoConsumo , 
    TCB.puntoConsumoLabelColor, TCB.puntoConsumoLabelBGColor)
  };
    ...
    try {
      ...
      if (respTerritorio.status === 200) {
        ...
        if ( jsonTerritorio.address.country === 'España') {
          ...
          status = {zona: territorio, calle: jsonTerritorio.address.road, ciudad: jsonTerritorio.address.city} ;
        } else {
...
function inclinacionTejado( cell) {
  ...
  activo.objeto.areaReal = activo.objeto.areaMapa / Math.cos(activo.objeto.inclinacionTejado / 180 * Math.PI);
...
function borraObjeto(cell){
  ...
  const idxObjetoActivo = TCB[activo.nombreTabla].findIndex( obj => obj[_id] === activo.id); //Posicion en la tabla para usar luego en la función splice

  if (activo.nombreTabla === "PuntoConsumo") { //Vamos a verificar que no tiene Fincas asociadas
    let fincasEnPuntoConsumo = UTIL.selectTCB("Finca", 'idPuntoConsumo', activo.objeto.idPuntoConsumo);
    if (fincasEnPuntoConsumo.length > 0) {
      if (confirm( TCB.i18next.t( "hay fincas - borrarlas?"))) { //Pedimos confirmacion
  ...
  if (Object.prototype.hasOwnProperty.call(activo.objeto, "geometria")) {
    for (const geoProp in activo.objeto.geometria) TCB.origenDatosSolidar.removeFeature(activo.objeto.geometria[geoProp]);
  }
