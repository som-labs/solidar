...
const TiltStep = () => {
  ...
  function setTilt(index, value) {
    ...
    oldBases[nIndex].areaReal =
      TCB.BaseSolar[nIndex].areaMapa / Math.cos((value / 180) * Math.PI)
    oldBases[nIndex].potenciaMaxima =
      oldBases[nIndex].areaReal / TCB.parametros.conversionAreakWp

    TCB.BaseSolar[nIndex].inclinacionTejado = value
    TCB.BaseSolar[nIndex].areaReal =
      TCB.BaseSolar[nIndex].areaMapa / Math.cos((value / 180) * Math.PI)
    TCB.BaseSolar[nIndex].potenciaMaxima =
      oldBases[nIndex].areaReal / TCB.parametros.conversionAreakWp
