...
class Graficos {
  ...
  i18nextGraficos() {
        ...
        annotations: [
          {
            ...
            text: TCB.i18next.t('graficos_LBL_maxConsumoMes', {
              maxConsumoMes: this.maxConsumoMes.toFixed(2),
            }),
            showarrow: true,
            arrowhead: 3,
            xanchor: 'center',
    ...
    if (this.divConsumosVersusGeneracion !== undefined) {
      layout_update = {
        title: TCB.i18next.t('graficos_LBL_produccionMediaMensual', {
    ...
    if (this.divConsumoGeneracion3D !== undefined) {
      ...
      data_update = {
        y: [
          TCB.produccion.idxTable.map((e) => {
            return e.dia + ' - ' + TCB.i18next.t(UTIL.nombreMes[e.mes])
          }),
          TCB.consumo.idxTable.map((e) => {
            return e.dia + ' - ' + TCB.i18next.t(UTIL.nombreMes[e.mes])
          }),
        ],
        name: [
        ...
        hovertemplate: [
          TCB.i18next.t('graficos_LBL_graficasHora') +
            ': %{y}<br>' +
            TCB.i18next.t('graficos_LBL_graficasHora') +
            ': %{x}<br>' +
            TCB.i18next.t('graficos_LBL_graficasProduccion') +
    ...
    if (this.divResPerfilDiario !== undefined) {
      layout_update = {
        title:
          ...
          this.fecha[0] +
          ' - ' +
          TCB.i18next.t(UTIL.nombreMes[parseInt(this.fecha[1]) - 1]),
        xaxis: { title: TCB.i18next.t('graficos_LBL_graficasHora'), dtick: 4 },
        yaxis: {
          title: 'kWh',
          showline: true,
    ...
    if (this.divBalanceProduccion !== undefined) {
      layout_update = {
        title: TCB.i18next.t('graficos_LBL_balanceProduccion', {
          potencia: UTIL.formatoValor('potencia', TCB.produccion.potenciaTotal),
        }),
      }
      data_update = {
    ...
    if (this.divBalanceConsumo !== undefined) {
      layout_update = {
        title: TCB.i18next.t('graficos_LBL_balanceConsumo', {
          potencia: UTIL.formatoValor('potencia', TCB.produccion.potenciaTotal),
        }),
      }
      data_update = {
    ...
    if (this.divBalanceEconomico !== undefined) {
      ...
      if (TCB.modoActivo === INDIVIDUAL) {
        titulo = TCB.i18next.t('graficos_LBL_tituloBalanceEconomicoGlobal', {
          potencia: UTIL.formatoValor('potencia', TCB.produccion.potenciaTotal),
        })
      } else if (this.finca === undefined) {
        titulo =
          'Global: ' +
          TCB.i18next.t('graficos_LBL_tituloBalanceEconomicoGlobal', {
            potencia: UTIL.formatoValor('potencia', TCB.produccion.potenciaTotal),
          })
      } else {
        const _potenciaAsignada =
          (TCB.produccion.potenciaTotal * this.finca.coefEnergia) / 100
        titulo =
          this.finca.nombreFinca +
          ': ' +
          TCB.i18next.t('graficos_LBL_tituloBalanceEconomicoFinca', {
    ...
    if (this.divPlotAlternativas !== undefined) {
      layout_update = {
        title: TCB.i18next.t('graficos_LBL_alternativasPotencia', {
          potencia: UTIL.formatoValor('potencia', this.potencia_kWp),
        }),
        annotations: [
          {
            x: TCB.totalPaneles,
  ...
  gestionTipoConsumo_MapaConsumo(consumo, donde1, donde2) {
    ...
    this.g_gestionTipoConsumo_MapaConsumo = Plotly.react(
      donde1,
      [g_consumo],
      layout_resumen,
    )
  ...
  gestionResultados_ConsumoGeneracion3D(donde) {
    ...
    gd.on('plotly_click', function (data) {
      this.fecha = data.points[0].y.split('/').map((a) => {
        return parseInt(a)
      })
  ...
  gestionResultados_TartaGrupos(donde) {
    ...
    let tipos = Finca.getGrupos.values.map((a) => {
      return a
    })
    let _pos
    for (let finca of TCB.Participes) {
      if (finca.grupo === 'Zonas Comunes') {
        ...
        datos[_pos - 1] = UTIL.selectTCB(
          'TipoConsumo',
          'nombreTipoConsumo',
          finca.nombreTipoConsumo,
        )[0].cTotalAnual
      } else if (finca.nombreTipoConsumo !== 'Participe sin consumo') {
        let _pos = Finca.getGrupos.values.findIndex((grupo) => {
          return finca.grupo === grupo
        })
        datos[_pos] += UTIL.selectTCB(
          'TipoConsumo',
          'nombreTipoConsumo',
          finca.nombreTipoConsumo,
        )[0].cTotalAnual
