...
async function GeneraInformePDF() {
  ...
  // await Plotly.toImage('graf_1', { format: 'png', width: 800, height: 500 }).then(
  //   function (dataURL) {
  ...
  nuevaLinea(
    'Dato',
    i++,
    'precioInstalacion_LBL',
    UTIL.formatoValor('dinero', TCB.economico.precioInstalacion),
  )
  nuevaLinea(
    'Dato',
    i++,
    'precioInstalacionCorregido_LBL',
...
function nuevaLinea(tipo, linea, propiedad, valor) {
  ...
  switch (tipo) {
    case 'Cabecera':
      ...
      doc.text(
        TCB.i18next.t('nombreProyecto_LBL') + ' ' + TCB.nombreProyecto,
        margenIzquierdo,
        currentY,
      )
      break
